...
%`
  ...
  it('adds line numbers when enabled', () => {
    const code = `G1 X10 Y20`,
      program = parse(code),
      formatter = new GCodeFormatter({ addLineNumbers: true }),
      traverser = new AstTraverser(formatter),
      formatted = formatter.formatGCode(program, traverser);
...
%`);
    ...
    const program2 = parse(formatted),
      formatter2 = new GCodeFormatter(),
      traverser2 = new AstTraverser(formatter2);
...
G00 Z22.0`,
      ...
      emptyLineIndex = lines.findIndex(
        (line, idx) =>
          line.trim() === '' &&
          idx > 0 &&
...
`,
      program = parse(code),
      formatter = new GCodeFormatter({ compactOutput: true }),
      traverser = new AstTraverser(formatter),
      formatted = formatter.formatGCode(program, traverser);
