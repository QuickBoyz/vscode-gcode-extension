...
describe('HaasProvider', () => {
  ...
  describe('DocumentFormattingProvider', () => {
    ...
    it('formats N-block labels (Haas uses N instead of O)', () => {
      const input = 'N100 #<x> = 5';
      const document = TextDocument.create('test.nc', 'gcode', 1, input);
      ...
      expect(edits).toHaveLength(1);
      expect(edits[0].newText).toContain('N100');
    });
  ...
  describe('document range formatting', () => {
    ...
    it('respects Haas control flow keywords in range', () => {
  ...
  describe('Dialect Parameter Handling', () => {
    it('uses Haas dialect when specified', () => {
