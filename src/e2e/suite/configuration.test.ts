...
suite("Configuration Tests", () => {
  ...
  test("Should respect addLineNumbers setting", async () => {
    ...
    assert.ok(
      formattedText.includes("N10"),
      "Should add line numbers when enabled"
    );
  });
  ...
  test("Should respect compactOutput setting", async () => {
    ...
    const doubleNewlines = formattedText.match(/\n\n\n/g);
    assert.ok(
  ...
  test("Should use default settings when not configured", async () => {
    ...
    assert.strictEqual(
      config.get("formatter.indent"),
      true,
      "Default indent should be true"
    );
  });
  ...
  test("Should apply multiple configuration changes", async () => {
    ...
    assert.ok(
      ...
      "Should apply formatting with all settings"
    );
  });
