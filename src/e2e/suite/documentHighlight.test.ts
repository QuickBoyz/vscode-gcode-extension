...
suite("Document Highlight Tests", () => {
  ...
  test("Should highlight all occurrences of variable", async function () {
    ...
    highlights!.forEach((highlight) => {
      ...
      assert.ok(
        highlightedText.includes("#<counter>"),
        "Should highlight variable name"
      );
    });
  });
  ...
  test("Should highlight variable in expressions", async function () {
    ...
    assert.ok(
      highlights!.length >= 2,
      "Should highlight variable in multiple expressions"
    );
  });
  ...
  test("Should not highlight at non-variable position", async function () {
    ...
    if (Array.isArray(highlights) && highlights.length > 0) {
      ...
      const text = document.getText();
      const hasVariableHighlight = highlights.some((highlight) => {
        const highlightedText = text.substring(
          document.offsetAt(highlight.range.start),
          document.offsetAt(highlight.range.end)
        );
        return (
          highlightedText.includes("#") || highlightedText.includes("<")
        );
      });
  ...
  test("Should highlight different variable types", async function () {
    ...
    const highlights1 = await vscode.commands.executeCommand<
      vscode.DocumentHighlight[] | undefined
    >("vscode.executeDocumentHighlights", document.uri, position1);

    assert.ok(
      highlights1 && highlights1.length >= 2,
      "Should highlight #1 variable"
    );

    // Test highlighting #<var>
    const position2 = new vscode.Position(1, 2);

    const highlights2 = await vscode.commands.executeCommand<
      vscode.DocumentHighlight[] | undefined
    >("vscode.executeDocumentHighlights", document.uri, position2);

    assert.ok(
      highlights2 && highlights2.length >= 2,
      "Should highlight #<var> variable"
    );
  });
