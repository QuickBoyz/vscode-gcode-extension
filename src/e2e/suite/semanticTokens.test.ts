...
const fixtureName = 'simple.nc',
  ...
  TOKEN_TYPES = ['keyword', 'variable', 'number', 'comment', 'function', 'parameter', 'label'];
...
suite('Semantic Tokens Tests', () => {
  ...
  test('Should provide semantic tokens for G-code document', async () => {
    const document = await TestUtils.openGCodeDocument(fixtureName),
      tokens = await vscode.commands.executeCommand<vscode.SemanticTokens | undefined>(
    ...
    await TestUtils.withTestDocument(
      ...
      async (document) => {
        ...
        const decoded = decodeSemanticTokens(tokens.data),
          variableTokens = decoded.filter((t) => t.tokenType === 'variable');
        ...
        const keywordTokens = decoded.filter((t) => t.tokenType === 'keyword');
        assert.ok(keywordTokens.length > 0, 'Should identify G1 as keyword');
      },
  ...
  test('Should identify G and M codes', async () => {
    await TestUtils.withTestDocument(
      ...
      async (document) => {
        ...
        const parameterTokens = decoded.filter((t) => t.tokenType === 'parameter');
        assert.ok(
        ...
        const numberTokens = decoded.filter((t) => t.tokenType === 'number');
        assert.ok(numberTokens.length > 0, 'Should identify number tokens');
      },
  ...
  test('Should identify comments', async () => {
    await TestUtils.withTestDocument(
      ...
      async (document) => {
        ...
        const decoded = decodeSemanticTokens(tokens.data),
          commentTokens = decoded.filter((t) => t.tokenType === 'comment');
        ...
        const keywordTokens = decoded.filter((t) => t.tokenType === 'keyword');
        assert.ok(keywordTokens.length > 0, 'Should identify G01 as keyword');
      },
