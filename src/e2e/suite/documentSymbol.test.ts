...
suite("Document Symbol Tests", () => {
  ...
  test("Should provide document symbols for variables", async () => {
    ...
    const variableSymbols = symbols!.filter((s) =>
      s.name.includes("#<")
    );
    assert.ok(
      variableSymbols.length > 0,
      "Should identify variable symbols"
    );
  });
  ...
  test("Should provide correct symbol locations", async () => {
    ...
    const counterSymbol = symbols!.find((s) =>
      s.name.includes("counter")
    );
    assert.ok(counterSymbol, "Should find counter symbol");
    assert.ok(
      counterSymbol!.range.start.line === 0,
      "Counter should be on line 0"
    );

    const varSymbol = symbols!.find(
      (s) => s.name.includes("var") && !s.name.includes("counter")
    );
    assert.ok(varSymbol, "Should find var symbol");
    assert.ok(
      varSymbol!.range.start.line === 1,
  ...
  test("Should handle documents with no variables", async () => {
    ...
    assert.ok(
      ...
      `Should return undefined or array. Got: ${typeof symbols}, value: ${symbols}`
    );
  ...
  test("Should provide symbols for complex documents", async () => {
    ...
    assert.ok(
      ...
      "Should return symbols for complex document"
    );
    assert.ok(
      symbols!.length > 0,
      "Should identify symbols in complex document"
    );
  });
