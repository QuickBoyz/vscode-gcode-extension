...
suite("Extension Activation Tests", () => {
  suiteSetup(async function () {
    ...
    if (!extension.isActive) {
      throw new Error(
        "Extension did not activate after opening G-code file"
      );
    }
  ...
  test("Language server should initialize", async () => {
    ...
    assert.ok(
      ...
      "Document symbol provider should be available"
    );
  });
  ...
  test("Extension should provide format command", async () => {
    ...
    const hasFormatCommand = commands.includes("gcode.formatDocument");
    ...
    const hasFormatProvider = hasFormatCommand || edits !== undefined;
    assert.strictEqual(
      hasFormatProvider,
      true,
