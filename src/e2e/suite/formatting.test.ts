...
suite("Document Formatting Tests", () => {
  ...
  test("Should format a simple G-code document", async () => {
    ...
    assert.ok(
      formattedText.includes("X10.0"),
      "Should format X10 to X10.0"
    );
  });

  test("Should respect prettyPrintCommands setting", async () => {
    await TestUtils.updateConfiguration(
    ...
    const formattedText = document.getText();
    assert.ok(
      formattedText.includes("G01"),
      "Should format G1 to G01 when enabled"
    );
    assert.ok(
      formattedText.includes("M03"),
      "Should format M3 to M03 when enabled"
    );
  });
  ...
  test("Should respect prettyPrintNumbers setting", async () => {
    ...
    assert.ok(
      formattedText.includes("X10.0"),
      "Should format X10 to X10.0 when enabled"
    );
    assert.ok(
      formattedText.includes("Y20.0"),
      "Should format Y20 to Y20.0 when enabled"
    );
  });
  ...
  test("Should respect indent setting", async () => {
    ...
    const g1Line = lines.find(
      (l) => l.includes("G1") || l.includes("G01")
    );
    assert.ok(g1Line, "Should find G1 line");
    assert.ok(
      g1Line!.startsWith("  ") || g1Line!.startsWith("\t"),
      "G1 line should be indented"
    );
  });
  ...
  suite("Range Formatting Tests", () => {
    test("Should format selected range", async () => {
      ...
      const edits = await vscode.commands.executeCommand<
        vscode.TextEdit[] | undefined
      >("vscode.executeFormatRangeProvider", document.uri, range);
