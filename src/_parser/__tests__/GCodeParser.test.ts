...
describe("GCodeParser", () => {
  ...
  it("parses a simple variable assignment", () => {
    ...
    expect((stmt as VariableAssignmentNode).value).toHaveProperty(
      "value",
      "10"
    );
  });
...
o100 endwhile
  ...
  it("parses a G-code command with axis parameters", () => {
    ...
    expect((params[0] as AxisParameterNode).axis).toBe("X");
  });
  ...
  it("parses a G-code with decimal code number", () => {
    ...
    expect(params[0]).toBeInstanceOf(AxisParameterNode);
    expect((params[0] as AxisParameterNode).axis).toBe("P");
    expect((params[0] as AxisParameterNode).value).toHaveProperty(
      "value",
      "1000"
    );
  });
...
#<y> = ABS[#<x>]
    ...
    expect(program.statements[2]).toBeInstanceOf(
      VariableAssignmentNode
    );
  });
});
...
G00 X#<x> Y#<y> F100
    ...
    expect(program.statements[1]).toBeInstanceOf(
      VariableAssignmentNode
    );
    expect(program.statements[2]).toBeInstanceOf(
      VariableAssignmentNode
    ...
    const axisX = cmd.getParameters()[0] as AxisParameterNode;
...
`);
    ...
    const ifStmt = program.statements[0] as IfStatementNode;
...
`);
    ...
    const whileStmt = program.statements[0] as WhileStatementNode;
    const outer = whileStmt.body[0] as IfStatementNode;
  ...
  it("creates ErrorNode for invalid syntax", () => {
    ...
    expect((errorNode as ErrorNode).message).toMatch(
      /Unexpected token/
    );
  });
...
M30
  ...
  describe("numeric variable parsing", () => {
    ...
    it("parses single-digit numeric variables as numbers", () => {
      ...
      const stmt = program.statements[0] as VariableAssignmentNode;
      const varRef = stmt.value as VariableReferenceNode;
      expect(typeof varRef.name).toBe("number");
    ...
    it("parses numeric variables in axis parameters", () => {
      ...
      const xParam = cmd.getParameters()[0] as AxisParameterNode;
      const xValue = xParam.value;
    ...
    it("parses numeric variables in conditional expressions", () => {
      ...
      const ifStmt = program.statements[0] as IfStatementNode;
      const condition = ifStmt.ifClause.condition;
    ...
    it("distinguishes between numeric and named variables", () => {
      ...
      const namedValue = namedStmt.value as VariableReferenceNode;
      expect(typeof namedValue.name).toBe("string");
      expect(namedValue.name).toBe("x");

      const numericStmt = program
        .statements[1] as VariableAssignmentNode;
      const numericValue = numericStmt.value as VariableReferenceNode;
