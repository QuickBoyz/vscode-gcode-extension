...
import { hitTestProjectedFrame, ProjectedFrame } from '../hitTesting';
...
/**
 * rAF-gated hit testing against the projected frame view.
 * Calls `onHover` when the hovered segment changes.
 */
export function useHitTesting(
  getProjectedFrame: () => ProjectedFrame,
  ...
  const processHitTest = useCallback(() => {
    ...
    const hit = hitTestProjectedFrame(
